   ...
   @exec command, (error, stdout) ->
     ...
     @exec command_m, (error, stdout_m) ->
       ...
       @exec command_n, (error, stdout_n) ->
         ...
         if ttl_gap > 0
           msg.send "Masato: #{ttl_m}¥n" + "Nina: #{array_n}¥n" + "Nina owes Masato #{ttl_gap}"
         else if ttl_gap < 0
           msg.send "Masato: #{ttl_m}¥n" + "Nina: #{array_n}¥n" + "Masato owes Nina #{ttl_gap * -1}"
         else if ttl_gap == 0
           msg.send "Masato: #{ttl_m}¥n" + "Nina: #{array_n}¥n" + "No money moevment"
         else
